...
frontend_splash = FRONTEND_BUILD_DIR / "static" / "splash.mp4"

if frontend_splash.exists():
    try:
        shutil.copyfile(frontend_splash, STATIC_DIR / "splash.mp4")
    except Exception as e:
        logging.error(f"An error occurred: {e}")
else:
    ...
    try:
        ...
        if r.ok:
            ...
            if "splash" in data:
                ...
                r = requests.get(url, stream=True)
                if r.status_code == 200:
                    with open(f"{STATIC_DIR}/splash.mp4", "wb") as f:
                        r.raw.decode_content = True
                        shutil.copyfileobj(r.raw, f)
...
if ENV == "prod":
    if OLLAMA_BASE_URL == "/ollama" and not K8S_FLAG:
        if USE_OLLAMA_DOCKER.lower() == "true":
            # if you use all-in-one docker container (ThrallPortal + Ollama)
...
DEFAULT_ARENA_MODEL = {
    ...
    "meta": {
        ...
        "description": "Ask your questions here in the portal... you are anonymous!",
        "model_ids": None,
    },
}
